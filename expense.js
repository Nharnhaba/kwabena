  ...
  notifListener = db.collection("notifications")
    ...
    .onSnapshot((snap) => {
      ...
      if (notifs.length > 0) {
        ...
        const sysNotifKey = `shown_sys_notif_${latestNotif.id}`;
        const alreadyShown = localStorage.getItem(sysNotifKey) === "true";
        
        if (!alreadyShown) {
          localStorage.setItem(sysNotifKey, "true");
          
          // Only show system notification if the message is less than 24 hours old
          const notifTime = latestNotif.createdAt?.toDate ? latestNotif.createdAt.toDate().getTime() : Date.now();
          ...
          if (ageMs < maxAgeMs) {
            ...
            if (typeof showBudgetToast === "function") {
              showBudgetToast(`New broadcast: ${latestNotif.title}`, "ok");
            }
            if ("Notification" in window && Notification.permission === "granted" && localStorage.getItem("sika-notifications-enabled") !== "false") {
              const title = latestNotif.title || "New Message";
              const options = {
                body: latestNotif.message || "",
                icon: "icon-192.png",
                tag: "sika-notif-" + latestNotif.id
              };
              if (navigator.serviceWorker && navigator.serviceWorker.ready) {
                navigator.serviceWorker.ready.then(swReg => {
                  swReg.showNotification(title, options);
